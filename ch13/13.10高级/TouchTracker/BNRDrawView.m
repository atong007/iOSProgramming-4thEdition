...
- (void)pan:(UIPanGestureRecognizer *)gr
{
    ...
    if (gr.state == UIGestureRecognizerStateChanged) {
        ...
        int distance = MAX(velocity.x, velocity.y) / 10;
        lineWith = distance + lineWith;
        NSLog(@"lineWith:%d", lineWith);
        // 将拖移距离加至选中的线条的起点和终点
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (BNRLine *line in self.finishedLines) {
        [self strokeLine:line withWidth:5];
    }
    [[UIColor redColor] set];
    for (NSValue *key in self.linesInProgress) {
        [self strokeLine:self.linesInProgress[key] withWidth:5];
    }
    
    if (self.selectedLine) {
        [[UIColor greenColor] set];
        [self strokeLine:self.selectedLine withWidth:lineWith];
        lineWith = 5;
    }
}

- (void)strokeLine:(BNRLine *)line withWidth:(int)width
{
