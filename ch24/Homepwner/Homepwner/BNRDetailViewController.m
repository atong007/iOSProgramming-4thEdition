...
@interface BNRDetailViewController () <UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIViewControllerRestoration>
...
- (void)encodeRestorableStateWithCoder:(NSCoder *)coder
{
    ...
    [[BNRItemStore sharedInstance] saveChanges];
...
- (void)decodeRestorableStateWithCoder:(NSCoder *)coder
{
    ...
    for (BNRItem *item in [[BNRItemStore sharedInstance] allItems]) {
...
- (IBAction)showAssetTypePicker:(id)sender
{
    ...
    assetTypeVC.modalPresentationStyle = UIModalPresentationPopover;
    assetTypeVC.popoverPresentationController.sourceView = self.toolBar;
    
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        __weak typeof(self) weakSelf = self;
        assetTypeVC.dismissAction = ^{
            __strong typeof(self) strongSelf = weakSelf;
            NSString *typeLabel = [strongSelf.item.assetType valueForKey:@"label"];
            if (!typeLabel) {
                typeLabel = @"None";
            }
            
            strongSelf.assetTypeButton.title = [NSString stringWithFormat:@"Type:%@", typeLabel];
        };
        [self presentViewController:assetTypeVC animated:YES completion:nil];
    }else {
