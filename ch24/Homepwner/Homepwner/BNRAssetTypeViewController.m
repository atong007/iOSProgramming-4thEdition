...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [[[BNRItemStore sharedInstance] allAssetTypes] count] + 1;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //2.传递数据模型来设置cell属性
    if (indexPath.row == [[[BNRItemStore sharedInstance] allAssetTypes] count]) {
        NSArray *array = [[BNRItemStore sharedInstance] getAllItemsWithSameAssetType:self.item.assetType];
        NSString *string = @"";
        for (BNRItem *item in array) {
            string = [string stringByAppendingString:[NSString stringWithFormat:@"%@ ", item.itemName]];
        }
